...
export function UnscheduledRow({ features }: UnscheduledRowProps) {
    ...
    <div className="flex items-center gap-2 py-2 px-2 bg-amber-50 border-t border-amber-200">
      ...
      <div className="flex flex-wrap gap-1">
        {features.map((feature) => (
          <span
            key={feature.id}
            className={`text-[10px] px-2 py-0.5 rounded ${statusColors[feature.status]}`}
            title={`${feature.name} - needs dates`}
          >
            {feature.name}
          </span>
        ))}
